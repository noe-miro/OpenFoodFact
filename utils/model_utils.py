...
class Process_data(BaseEstimator, TransformerMixin):
    ...
    def get_embeddings_raw(self, path):
        self._embeddings = self._load_emb(path)


    def get_embeddings_light(self, path):
        self._embeddings_light = self._load_emb(path)
    ...
    def transform(self, path_to_file):
        ...
        img_cosine_sim = cosine_similarity(self._embeddings, img_emb.reshape(1,-1))

        return self._filenames, img_cosine_sim.flatten()
    ...
    def generation_df(self, bool_etiquette):
        ...
        dataframe=[]
        for filename, label, emb in zip(self._filenames, self._kmeans.labels_, self._embeddings_proj3D):
            label_class = class_dict[label]
